...
def bresenham2(x1, y1, x2, y2):
    ...
    while t < el:
        if error > w:
            ...
            error += m
        t += 1
        setPixel(x, y, round(255 * (1 - (error / 255))), round(255 * (1 - (error / 255))),
                 round(255 * (1 - (error / 255))))
...
def makeLines():
    for i in range(len(points)):
        if i == len(points) - 1:
            ...
            if (points[0][1] > points[i][1] and points[i - 1][1] > points[i][1]) or (
                    points[0][1] < points[i][1] and points[i - 1][1] < points[i][1]):
            ...
            if (points[1][1] > points[0][1] and points[len(points) - 1][1] > points[0][1]) or (
                    points[1][1] < points[0][1] and points[len(points) - 1][1] < points[0][1]):
                pointsScan[points[i][1]].append([points[i][0], 1])
            else:
                pointsScan[points[i][1]].append([points[i][0], 0])
        else:
            bresenham2(points[i][0], points[i][1], points[i + 1][0], points[i + 1][1])
            initPoints(points[i][0], points[i][1], points[i + 1][0], points[i + 1][1])
            if (points[i + 1][1] > points[i][1] and points[i - 1][1] > points[i][1]) or (
                    points[i + 1][1] < points[i][1] and points[i - 1][1] < points[i][1]):
                pointsScan[points[i][1]].append([points[i][0], 1])
            else:
                pointsScan[points[i][1]].append([points[i][0], 0])


def drawLines(x1, x2, y):
    xs = x1
    xf = x2
    if x1 > x2:
        xs = x2
    ...
    for i in range(len(pointsScan)):
        if len(pointsScan[i]) != 0:
            ...
            while j < len(pointsScan[i]):
                ...
                if (j + 1) % 2 != 0:
                    drawLines(pointsScan[i][j][0], pointsScan[i][j + 1][0], i)
                j += 1
...
def key_callback(window, key, scancode, action, mods):
    ...
    if action == glfw.PRESS:
        ...
        if key == glfw.KEY_BACKSPACE:
            ...
            a = []
            for i in range(len(pointsScan)):
                pointsScan[i] = deepcopy(a)
                pixels = [0] * pSize


def cursor_pos(window, xpos, ypos):
    global cursorPositionX, cursorPositionY
    cursorPositionX = xpos
    cursorPositionY = size - ypos
...
def mouse_button(window, button, action, mods):
    ...
    if action == glfw.PRESS:
        if button == glfw.MOUSE_BUTTON_LEFT:
            setPixel(round(cursorPositionX), round(cursorPositionY), 255, 255, 255)
            points.append([round(cursorPositionX), round(cursorPositionY)])


def main():
    if not glfw.init():
        return
    window = glfw.create_window(size, size, "4lab", None, None)
    if not window:
        glfw.terminate()
        return
